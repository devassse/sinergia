 ...
 * Wall Functions
 */
class Youzify_Activity_Functions {
	...
	function __construct( ) {
		...
		add_action( 'wp_ajax_youzify_get_url_live_preview', array( $this, 'get_live_url_preview' ) );
		...
		add_filter( 'youzify_activity_can_share', array( $this, 'set_shares_visibility' ) );
		...
		add_filter( 'youzify_activity_tools', array( $this, 'add_delete_activity_tool' ), 99, 2 );
	...
	function hide_emojis_from_content() {
        ...
        // Hide Posts Emoji
        if ( 'off' == youzify_option( 'youzify_enable_posts_emoji', 'on' ) ) {
            add_filter( 'bp_get_activity_content_body', 'youzify_remove_emoji' );
        }

        // Hide Comments Emoji
        if ( 'off' == youzify_option( 'youzify_enable_comments_emoji', 'on' ) ) {
            add_filter( 'bp_activity_comment_content', 'youzify_remove_emoji' );
        }
	...
	function allow_comments_without_content( $content, $activity ) {
		...
		if ( $activity->type == 'activity_comment' ) {
			$content = str_replace( '{{{youzify_comment_attachment}}}', '', $content );
		}
	...
	function embed_videos_container( $html, $url, $attr, $rawattr ) {
		...
		foreach ( $providers as $provider ) {
			if ( strpos( $url, $provider ) !== false ) {
				return '<div class="youzify-embed-wrapper">' . $html . '</div>';
			}
		}
	...
	function add_delete_activity_tool( $tools, $post_id ) {
		...
		$tools[] = array(
			...
			'class' => array( 'youzify-delete-tool', 'youzify-delete-post' ),
	...
	function wall_posts_per_page( $query ) {
		...
		if ( bp_is_activity_directory() ) {
			$posts_per_page = youzify_option( 'youzify_activity_wall_posts_per_page', 5 );
		} elseif( bp_is_user_activity() ) {
			$posts_per_page = youzify_option( 'youzify_profile_wall_posts_per_page', 5 );
		} elseif( bp_is_groups_component() ) {
			$posts_per_page = youzify_option( 'youzify_groups_wall_posts_per_page', 5 );
		} else {
			$posts_per_page = '';
		}
	...
	function get_show_everything_filter( $retval = null ) {
		...
		$filter_actions = apply_filters( 'youzify_wall_show_everything_filter_actions', $filter_actions );
	...
	function get_unallowed_post_types( $filters = null) {
		...
		$types = youzify_option( 'youzify_unallowed_activities' );
		...
		return apply_filters( 'youzify_wall_post_types_visibility', $types );
	}
	...
	function set_likes_visibility( $can ) {
		return 'off' == youzify_option( 'youzify_enable_wall_posts_likes', 'on' ) ? false : $can;
	}
	...
	function set_comments_visibility( $can ) {
		return 'off' == youzify_option( 'youzify_enable_wall_posts_comments', 'on' ) ? false : $can;
	}
	...
	function set_shares_visibility( $can ) {
		return 'off' == youzify_option( 'youzify_enable_wall_posts_shares', 'on' ) ? false : $can;
	}
	...
	function set_replies_visibility( $can ) {
		return 'off' == youzify_option( 'youzify_enable_wall_posts_reply', 'on' ) ? false : $can;
	}
	...
	function set_delete_visibility( $can ) {
		return 'off' == youzify_option( 'youzify_enable_wall_posts_deletion', 'on' ) ? false : $can;
	}
	...
	function enable_posts_embeds( $enable ) {
		return 'off' == youzify_option( 'youzify_enable_wall_posts_embeds', 'on' ) ? false : $enable;
	}
	...
	function enable_comments_embeds( $enable ) {
		return 'off' == youzify_option( 'youzify_enable_wall_comments_embeds', 'on' ) ? false : $enable;
	}
	...
	function hide_private_users_posts( $where ) {
	    ...
	    if (  is_super_admin( bp_loggedin_user_id() ) || 'off' == youzify_option( 'youzify_allow_private_profiles', 'off' ) ) {
			return $where;
	    }

	    // Get List of Private Users.
	    $private_users = youzify_get_private_user_profiles();
	...
	function get_live_url_preview() {
		...
		// Include Live Preview Class
		include_once YOUZIFY_CORE . 'functions/live-preview/classes/LinkPreview.php';
	...
	function show_activity_tools_icon() {
		...
		if ( ! apply_filters( 'youzify_display_activity_tools', is_user_logged_in() ) ) {
			return;
		}
	...
	function change_activity_read_more_text() {
	    return __( 'Read More', 'youzify' );
	}

}

new Youzify_Activity_Functions();